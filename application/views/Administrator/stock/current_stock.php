				...
				<table class="table table-bordered" v-if="searchType == 'current'" style="display:none" v-bind:style="{display: searchType == 'current' ? '' : 'none'}">
					...
					<tbody>
						<tr v-for="product in stock">
							...
							<td v-if="product.Unit_Name == 'SFT'" style="text-align:center;">{{ parseInt(product.current_quantity/product.roll_per_sft) + ' Roll' }} {{ product.current_quantity % product.roll_per_sft + ' SFT' }}</td>

							<td v-else style="text-align:center;">{{ product.current_quantity }} {{ product.Unit_Name }}</td>
