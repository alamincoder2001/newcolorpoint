					...
					<div class="row">
						...
						<div class="col-sm-6">
							<form v-on:submit.prevent="addToCart">
								...
								<div class="form-group" style="display:none" :style="{display:selectedProduct.Unit_Name == 'SFT' ? '' : 'none'}">
									<label class="col-xs-4 control-label no-padding-right"> Size Details</label>
									<div class="col-xs-8">
										<input type="text" class="form-control" v-model="size_details" disabled />
									</div>
								</div>
								...
								<div class="form-group" style="display:none" :style="{display:selectedProduct.Unit_Name == 'SFT' ? '' : 'none'}">
									<label class="col-xs-4 control-label no-padding-right"> Roll </label>
									<div class="col-xs-8">
										<div class="row">
											<div class="col-xs-4 no-padding-right">
												<input type="text" step="0.01" v-model.number="roll" class="form-control" v-on:input="productTotal" />
											</div>
											<div class="col-xs-3" style="text-align: right;">SFT</div>
											<div class="col-xs-5 no-padding-left">
												<input type="text" step="0.01" v-model.number="sft" class="form-control" v-on:input="productTotal" />
											</div>
										</div>
									</div>
								</div>
...
<script>
	...
	new Vue({
		...
		// watch: {
		// 	'selectedProduct.Unit_Name'(value) {
		// 		axios.post('/get-sizes', {
		// 			status: 'a'
		// 		}).then(res => {
		// 			this.sizes = res.data.filter((obj) => {
		// 				return obj.Unit_Name == value
		// 			});
		// 		})
		// 	}
		// },
		methods: {
			...
			onChangeProduct() {
				...
				this.size_details = this.selectedProduct.size_name + ' = ' + this.selectedProduct.roll_per_sft + ' SFT'
				// this.selectedSize = {
				// 	size_id: '',
				// 	size_name: 'Select---'
				// }
			...
			productTotal() {
				...
				if (this.selectedProduct.Unit_Name == 'SFT') {
					// console.log(this.roll, this.selectedProduct.roll_per_sft, this.sft, this.selectedProduct.Product_Purchase_Rate);
					this.selectedProduct.quantity = (this.roll * this.selectedProduct.roll_per_sft) + +this.sft;
					this.selectedProduct.total = ((this.roll * this.selectedProduct.roll_per_sft) + +this.sft) * this.selectedProduct.Product_Purchase_Rate;
				} else {
					// console.log(this.selectedProduct.quantity, this.selectedProduct.Product_Purchase_Rate);
					let total = this.selectedProduct.quantity * this.selectedProduct.Product_Purchase_Rate;
					this.selectedProduct.total = total;
