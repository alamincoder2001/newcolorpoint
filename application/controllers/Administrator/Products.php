...
class Products extends CI_Controller
{
    ...
    public function addProduct()
    {
        ...
        try {
            ...
            $productNameCount = $this->db->query("select * from tbl_product where Product_Name = ? and size_id = ?", [$productObj->Product_Name, $productObj->size_id])->num_rows();
            if ($productNameCount > 0) {
                $res = ['success' => false, 'message' => 'Product name with size already exists'];
    ...
    public function updateProduct()
    {
        ...
        try {
            ...
            $productNameCount = $this->db->query("select * from tbl_product where Product_Name = ? and size_id = ? and Product_SlNo != ?", [$productObj->Product_Name, $productObj->size_id, $productObj->Product_SlNo])->num_rows();
            if ($productNameCount > 0) {
                $res = ['success' => false, 'message' => 'Product name with same size already exists'];
                echo json_encode($res);
                exit;
            }
    ...
    public function getProducts()
    {
        ...
        $products = $this->db->query("
            select
                p.*,
                concat(p.Product_Code,' - ',p.Product_Name, ' - ', s.size_name) as display_text,
            ...
            left join tbl_sizes s on s.size_id = p.size_id
            where p.status = 'a'
            $clauses
    ...
    public function getTotalStock()
    {
        ...
        $stock = $this->db->query("
            select
                ...
                s.size_name,
                s.roll_per_sft,
                concat(p.Product_Name,' - ',s.size_name) as display_text,
