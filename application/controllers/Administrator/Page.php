...
class Page extends CI_Controller
{
    ...
    public function addSize()
    {
        ...
        $data['title'] = "Add size";
        $data['content'] = $this->load->view('Administrator/add_size', $data, TRUE);
        $this->load->view('Administrator/index', $data);
    }
    ...
    public function saveSize()
    {
        ...
        $sizeId = $inputObJ->size_id;

        if ($sizeId == '') {
            $query = $this->db->query("select * from tbl_sizes where size_name = ? and branch_id = ?", [$inputObJ->size_name, $this->brunch])->row();
        } else {
            $query = $this->db->query("select * from tbl_sizes where size_id != ? and size_name = ? and branch_id = ?", [$sizeId, $inputObJ->size_name, $this->brunch])->row();
        }

        if ($query) {
            $res = ['success' => false, 'message' => 'Duplicate Size Name'];
        ...
        try {
            ...
            if ($sizeId == '') {
                ...
                $this->db->where('size_id', $sizeId)->update('tbl_sizes', $data);
            }
    ...
    public function getSizes()
    {
        ...
        if (isset($data->status) && $data->status != '') {
            $clause = " where s.status = '$data->status' ";
        }
        // if (isset($data->memo_for) && $data->memo_for == 'payment') {
        //     $clause = " and memo_for = 'payment' ";
        // }

        $sizes = $this->db->query("SELECT s.*,u.Unit_Name
        FROM tbl_sizes s
        LEFT JOIN tbl_unit u on u.Unit_SlNo = s.unit_id
        $clause
        order by size_id asc")->result();
    ...
    public function deleteSize()
    {
        ...
        $this->db->where('size_id', $data->size_id)->update('tbl_sizes', $updateData);

        echo json_encode(['success' => true, 'message' => 'Size delete successfully']);
    }
