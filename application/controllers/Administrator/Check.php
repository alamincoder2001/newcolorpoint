...
//			print_r($this->session->userdata("cheque"));
	...
	public function check_paid_submission($cheque_id = Null){
		...
		$data = $this->db->query("SELECT * FROM tbl_checks where id=?", $cheque_id)->row();
		$clauses = " and c.Customer_SlNo = '$data->cus_id'";
		$dueResult = $this->mt->customerDue($clauses);

		//send message
		$currency = $this->session->userdata('Currency_Name');
		$message = "Dear {$dueResult[0]->Customer_Name},\nThanks for your payment by cheque. Received amount is {$currency} {$data->check_amount}. Current due is {$currency} {$dueResult[0]->dueAmount}";
		$recipient = $dueResult[0]->Customer_Mobile;
